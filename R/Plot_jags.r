...
print('here')
 ...
 print(x.matrix)
  ...
  y.outer.hiCI <- y.matrix%*%y.hiCI
    ...
    plot(x=x.points, y=y.points, xlim=(range(x.points)+ c(-1,1)*2.2*x.sd), ylim=range(y.points)+  c(-1,1)*2.2*y.sd,xlab=expression(paste(delta,C[13])), ylab=expression(paste(delta,N[15])),pch=c(19,19),col=c("grey","grey"))
...
curves.plot <- function(jags.1, num.sources, num.chains, color=FALSE,individuals, xlab.vec, num.groups=1) {
  ...
  for(j in 1:num.sources) {
	...
	if(num.groups > 1) {
		p.group <- which(out.names== paste("p.group[",j,']',sep='') )
	}
    ...
    for(i in 2:individuals) {
     ...
     if(color) { lines(temp.smooth, type='l', lwd=2, col='cornflowerblue') } else {
	...
	if(num.groups > 1) {
		...
		for(i in 1:num.groups) {
			...
			temp <- hist(mcmc.rbind[,p.group],plot=FALSE, breaks=seq(0,1,length.out=100))
			temp.smooth <- lowess(c(0,temp$mids,1), y= c(0,temp$density,0), f=.1)
			temp.smooth$x <- temp.smooth$x[-c(1,length(temp.smooth$x))]
			temp.smooth$y <- temp.smooth$y[-c(1,length(temp.smooth$y))]
			
			
			if(color) { lines(temp.smooth, type='l', lwd=2, col='firebrick2') } else {
			lines(temp.smooth, type='l', lwd=2, col="black")}    
		}
